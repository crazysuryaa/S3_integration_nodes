...
class S3TextNode:
    """Unified S3 Text Node for loading and saving text files"""
        ...
        return {
            "required": {
                ...
                "s3_path": ("STRING", {
                    "default": "",
                    "multiline": False,
                    "placeholder": "path/to/text.txt"
                })
            },
            "optional": {
                "text": ("STRING", {"forceInput": True}),  # Required for save operation
                "use_input_dir": ("BOOLEAN", {"default": True}),
                "use_output_dir": ("BOOLEAN", {"default": True}),
                "encoding": (["utf-8", "ascii", "latin-1", "utf-16"], {"default": "utf-8"}),
    ...
    RETURN_NAMES = ("text", "local_path", "s3_path")
    FUNCTION = "process_text"
    OUTPUT_NODE = True
    
    def process_text(self, operation, s3_path, text=None, use_input_dir=True, use_output_dir=True,
                    encoding="utf-8", keep_local_copy=False, delete_local=False,
        ...
        if operation == "load":
            return self._load_text(s3_path, use_input_dir, encoding, keep_local_copy, s3_client)
        elif operation == "save":
            if text is None:
                raise ValueError("Text input is required for save operation")
            return self._save_text(text, s3_path, use_output_dir, encoding, delete_local, s3_client)
        else:
            raise ValueError(f"Unknown operation: {operation}")
    
    def _load_text(self, s3_path, use_input_dir=True, encoding="utf-8", keep_local_copy=False, s3_instance=None):
        """Load text from S3"""
        ...
        try:
            ...
            if keep_local_copy:
                ...
                local_dir = os.path.join("temp", "text")
            ...
            logger.info(f"Downloading text file from S3: {full_s3_path}")
            downloaded_path = s3_instance.download_file(full_s3_path, local_path)
            
            if downloaded_path is None:
                raise RuntimeError(f"Failed to download text file from S3: {full_s3_path}")
            ...
            logger.info(f"Successfully loaded text from S3: {full_s3_path}")
            return (text_content, downloaded_path if keep_local_copy else "", full_s3_path)
            
        except Exception as e:
            error_msg = f"Failed to load text from S3 path '{s3_path}': {str(e)}"
            logger.error(error_msg)
            raise RuntimeError(error_msg)
    
    def _save_text(self, text, s3_path, use_output_dir=True, encoding="utf-8", delete_local=False, s3_instance=None):
        """Save text to S3"""
        ...
        try:
            ...
            file_extension = os.path.splitext(s3_path)[1] or '.txt'
            temp_file = tempfile.NamedTemporaryFile(delete=False, suffix=file_extension, mode='w', encoding=encoding)
            temp_path = temp_file.name
            temp_file.write(text)
            temp_file.close()
            
            # Upload to S3
            logger.info(f"Uploading text file to S3: {temp_path} -> {full_s3_path}")
            uploaded_path = s3_instance.upload_file(temp_path, full_s3_path)
            
            if uploaded_path is None:
                raise RuntimeError(f"Failed to upload text file to S3: {full_s3_path}")
            
            # Clean up or keep local file
            if delete_local:
                try:
                    os.remove(temp_path)
                    logger.info(f"Deleted local text file: {temp_path}")
                except Exception as e:
                    logger.warning(f"Failed to delete local text file: {e}")
            
            logger.info(f"Successfully saved text to S3: {uploaded_path}")
            return (text, temp_path if not delete_local else "", uploaded_path)
            
        except Exception as e:
            error_msg = f"Failed to save text to S3 path '{s3_path}': {str(e)}"
            logger.error(error_msg)
            raise RuntimeError(error_msg)
